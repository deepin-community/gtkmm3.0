...
// static
GType Glib::Value<Gdk::FrameClockPhase>::value_type()
{
...
{
  return Glib::RefPtr<Gdk::FrameClock>( dynamic_cast<Gdk::FrameClock*> (Glib::wrap_auto ((GObject*)(object), take_copy)) );
...
void FrameClock::request_phase(FrameClockPhase phase)
{
  gdk_frame_clock_request_phase(gobj(), ((GdkFrameClockPhase)(phase)));
}
...
{
  Glib::RefPtr<FrameTimings> retvalue = Glib::wrap(gdk_frame_clock_get_timings(gobj(), frame_counter));
  if(retvalue)
...
{
  Glib::RefPtr<FrameTimings> retvalue = Glib::wrap(gdk_frame_clock_get_current_timings(gobj()));
  if(retvalue)
    retvalue->reference(); //The function does not do a ref for us.
  return retvalue;
}

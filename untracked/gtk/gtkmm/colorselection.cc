...
ColorSelection::ColorSelection(ColorSelection&& src) noexcept
: Gtk::VBox(std::move(src))
{}

ColorSelection& ColorSelection::operator=(ColorSelection&& src) noexcept
{
  Gtk::VBox::operator=(std::move(src));
  return *this;
}

ColorSelection::~ColorSelection() noexcept
{
  destroy_();
}
...
void ColorSelection::set_current_rgba(const Gdk::RGBA& rgba)
{
  gtk_color_selection_set_current_rgba(gobj(), (rgba).gobj());
}

void ColorSelection::set_previous_rgba(const Gdk::RGBA& rgba)
{
  gtk_color_selection_set_previous_rgba(gobj(), (rgba).gobj());
}
...
Glib::SignalProxy< void > ColorSelection::signal_color_changed()
{
  return Glib::SignalProxy< void >(this, &ColorSelection_signal_color_changed_info);
}
...
ColorSelectionDialog::ColorSelectionDialog(ColorSelectionDialog&& src) noexcept
: Gtk::Dialog(std::move(src))
{}

ColorSelectionDialog& ColorSelectionDialog::operator=(ColorSelectionDialog&& src) noexcept
{
  Gtk::Dialog::operator=(std::move(src));
  return *this;
}

ColorSelectionDialog::~ColorSelectionDialog() noexcept
{
  destroy_();
}
...
:
  // Mark this class as non-derived to allow C++ vfuncs to be skipped.
  Glib::ObjectBase(nullptr),
  Gtk::Dialog(Glib::ConstructParams(colorselectiondialog_class_.init(), "title", title.c_str(), nullptr))
{

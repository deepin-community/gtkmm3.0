...
RecentChooserMenu::RecentChooserMenu(RecentChooserMenu&& src) noexcept
: Gtk::Menu(std::move(src))
  , RecentChooser(std::move(src))
  , Activatable(std::move(src))
{}

RecentChooserMenu& RecentChooserMenu::operator=(RecentChooserMenu&& src) noexcept
{
  Gtk::Menu::operator=(std::move(src));
  RecentChooser::operator=(std::move(src));
  Activatable::operator=(std::move(src));
  return *this;
}

RecentChooserMenu::~RecentChooserMenu() noexcept
{
  destroy_();
}
...
:
  // Mark this class as non-derived to allow C++ vfuncs to be skipped.
  Glib::ObjectBase(nullptr),
  Gtk::Menu(Glib::ConstructParams(recentchoosermenu_class_.init(), "recent_manager", Glib::unwrap(recent_manager), nullptr))
{

...
:
  // Mark this class as non-derived to allow C++ vfuncs to be skipped.
  Glib::ObjectBase(nullptr),
  Gtk::Container(Glib::ConstructParams(treeview_class_.init(), "model", Glib::unwrap(model), nullptr))
{
...
{
  Glib::RefPtr<Gdk::Window> retvalue = Glib::wrap(gtk_tree_view_get_bin_window(gobj()));
  if(retvalue)
    retvalue->reference(); //The function does not do a ref for us.
  return retvalue;
}
...
Cairo::RefPtr<Cairo::Surface> TreeView::create_row_drag_icon(const TreeModel::Path& path) const
{
  return Cairo::RefPtr<Cairo::Surface>(new Cairo::Surface(gtk_tree_view_create_row_drag_icon(const_cast<GtkTreeView*>(gobj()), const_cast<GtkTreePath*>((path).gobj())), true /* take reference */));
}

...
:
  gobject_ ((other.gobject_) ? gtk_icon_source_copy(other.gobject_) : nullptr)
{}

IconSource::IconSource(IconSource&& other) noexcept
:
...
IconSource& IconSource::operator=(IconSource&& other) noexcept
{
  IconSource temp (std::move(other));
  swap(temp);
  return *this;
}
...
IconSource::~IconSource() noexcept
{
  if(gobject_)
    gtk_icon_source_free(gobject_);
}

void IconSource::swap(IconSource& other) noexcept
{
